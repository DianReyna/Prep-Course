...
function numberOfCharacters(string) {
  ...
  //Escribe tu código aquí
  let cadena = {};
  let nueva = string.split("");

  for (let i = 0; i < string.length; i++) {
    letra = nueva[i].charCodeAt();
    cadena[letra] == undefined ? (cadena[letra] = 1) : cadena[letra]++;
  }
  let resultado = {};
  for (let i in cadena) resultado[String.fromCharCode(i)] = cadena[i];
  return resultado;
}
...
function buscoInterseccion(arreglo1, arreglo2) {
  ...
  for (let i = 0; i < arreglo1.length; i++) {
    objeto[arreglo1[i]] = true;
  }
  for (let j = 0; j < arreglo2.length; j++) {
    if (objeto[arreglo2[j]]) result.push(arreglo2[j]);
  }
  return result;
}
